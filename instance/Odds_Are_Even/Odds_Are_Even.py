...
def get_start(instance,dim):
    ...
    for i in range(dim*dim,len(instance)):
        start.append(instance[i])
    return start
...
def write_ins(file_name,b,dim,s):
    with open(file_name,"w") as f:
        ...
        t = np.array(b[dim*dim:2*dim*dim]).reshape(dim,dim)
        f.write("b=" + str(b1).replace("[[","[|").replace("]\n [","|").replace("  "," ").replace(" ",",").replace("]]","|]").replace("|,","|")+";\r\n")
        f.write("t=" + str(t).replace("[[","[|").replace("]\n [","|").replace("  "," ").replace(" ",",").replace("]]","|]").replace("|,","|")+";\r\n")
...
def main(b,game_name,dim,s,instance_number):
    ...
    if b != 0:
        beg = "PATH=$PATH:/home/jun2/Minizinc/bin && minizinc "
    else:
        beg = "minizinc "
    while ins_num <= instance_number:
        out = ""
        seed = random.randint(0,10000000)
        ##PATH=$PATH:/home/jun2/Minizinc/bin && 
        cmd = beg + "-r "+ str(seed) + r" --solver Gecode " + puzzle + " " + input
        ...
        cmd_solver = beg + "-r "+ str(seed) + r" -n 2 --solver Gecode " + solver + " " + path
        while out_num(cmd_solver) != 2:
            modify(b,instance)
            write_ins(path,b,dim,s)
        ins_num = ins_num+1
        print("No. " + str(ins_num) + " has done")


# In[19]:


main(1,"Odds_Are_Even",9,3,100)
